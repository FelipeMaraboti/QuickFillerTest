...
describe('Extractors', () => {
  ...
  it('deve extrair holerite separando fields de bases', () => {
    ...
    expect(pagina.fields[0].description).toContain('Dias Trabalhados')
    expect(pagina.fields[0].proventos).toBe('1.678,61')
    
    expect(pagina.fields[1].code).toBe('2007')
    expect(pagina.fields[1].description).toContain('Horas Extras 100%')
    expect(pagina.fields[1].proventos).toBe('76,30')
    ...
    expect(pagina.bases[2].label).toContain('Base I.N.S.S.')
    expect(pagina.bases[2].value).toBe('1.967,07')
    expect(pagina.bases[3].label).toContain('F.G.T.S. do Mês')
